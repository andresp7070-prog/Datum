...
- Reglas de inactividad (opcionales, por etapa): si un contacto lleva más de `dias_inactividad` sin ninguna interacción registrada, se mueve solo a `etapa_destino_inactividad_id`. No corre en segundo plano (no hay servidor propio para eso) — se aplica cada vez que alguien abre el CRM (`aplicar_reglas_inactividad_crm()`), así que el movimiento ocurre en la próxima visita a esa pantalla, no exactamente al minuto del vencimiento. Esto es lo que le da sentido al CRM para un negocio que vive de leads (que necesita darle seguimiento activo al embudo), a diferencia de uno que vive de ventas directas (que casi no lo va a usar, porque sus contactos nacen ya cerrados).
