...
export default async function FichaLeadPage({
  ...
  // Primera tanda: ninguna de estas depende del resultado de otra — todas
  // solo necesitan el id de la ruta o el usuario, ya conocido. Antes eran 9
  // viajes seguidos a la base de datos, ahora es 1.
  const [
    { data: lead },
    { data: responsables },
    { data: perfilAdmin },
    ...
    supabase
      ...
      .eq("id", id)
      .single(),
    supabase.from("datum_responsables").select("id, nombre").order("nombre"),
    ...
    user
      ? supabase.from("perfiles").select("empresa_id, empresas ( modulos_activos )").eq("id", user.id).single()
    ...
    supabase.from("vista_calificacion_leads_datum").select("calificacion_automatica").eq("lead_id", id).maybeSingle(),
  ...
  // Segunda tanda: dependen de resultados de la primera (perfilAdmin,
  // etapas + lead), también en paralelo entre sí.
  const [{ data: serviciosDisponibles }, { data: camposDefinidos }] = await Promise.all([
    perfilAdmin?.empresa_id && empresaAdminInfo?.modulos_activos?.includes("inventario")
      ? supabase.from("inventario_items").select("id, nombre").eq("empresa_id", perfilAdmin.empresa_id).order("nombre")
