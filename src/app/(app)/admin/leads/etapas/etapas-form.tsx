...
function CamposEtapaConfig({ etapaId, campos }: { etapaId: string; campos: CampoEtapa[] }) {
  ...
  async function agregar() {
    setError(null);
    setGuardando(true);
    const resultado = await crearCampoEtapaDatum({
      etapaId,
      nombre,
      tipo,
      opciones: opciones
  ...
  async function borrar(campoId: string) {
    ...
    await borrarCampoEtapaDatum(campoId);
    setBorrandoId(null);
    router.refresh();
  }
...
function EtapaFila({
  ...
  async function guardarNombre() {
    ...
    setError(null);
    setGuardando(true);
    const resultado = await renombrarEtapaDatum(etapa.id, nombre);
    setGuardando(false);
    if (resultado.error) {
      setError(resultado.error);
  ...
  async function mover(direccion: "arriba" | "abajo") {
    setGuardando(true);
    await moverEtapaDatum(etapa.id, direccion);
    setGuardando(false);
    router.refresh();
  }
  ...
  async function marcarCierre() {
    setGuardando(true);
    await marcarEtapaCierreDatum(etapa.id);
    setGuardando(false);
    router.refresh();
  }
  ...
  async function guardarRegla() {
    ...
    setGuardando(true);
    const resultado = await actualizarReglaInactividadDatum({
      etapaId: etapa.id,
  ...
  async function quitarRegla() {
    setGuardando(true);
    await actualizarReglaInactividadDatum({
      etapaId: etapa.id,
...
export function EtapasForm({
  ...
  async function agregar() {
    ...
    setGuardando(true);
    const resultado = await crearEtapaDatum(nuevaEtapa);
    setGuardando(false);
    if (resultado.error) {
      setError(resultado.error);
      return;
  ...
  return (
    <div className="max-w-3xl">
      ...
      <p className="mb-6 text-sm text-gray-500">
        Estas son las etapas de tu propio embudo de venta de Datum. Puedes agregar las que
        necesites, cambiarles el nombre y reordenarlas. La etapa marcada como &ldquo;de
        cierre&rdquo; es la que representa un lead ya convertido en cliente — solo puede haber
